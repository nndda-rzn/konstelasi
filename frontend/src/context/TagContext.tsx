...
export const UPDATE_TAG_MUTATION = gql`
  mutation UpdateTag($id: String!, $name: String, $color: String, $description: String) {
...
export const TagProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const { data, loading: queryLoading, error, refetch } = useQuery<any>(TAGS_QUERY, {
  ...
  const handleCreateTag = useCallback(async (name: string, color?: string, description?: string) => {
    await createTag({
      variables: {
        name,
        color,
  ...
  const handleUpdateTag = useCallback(async (id: string, name: string, color?: string, description?: string) => {
    await updateTag({
      variables: {
        id,
        name,
  ...
  const handleAssignTagsToNote = useCallback(async (noteId: string, tagIds: string[]) => {
  ...
  const handleRemoveTagFromNote = useCallback(async (noteId: string, tagId: string) => {
  ...
  const getTagSuggestions = useCallback((query: string) => {
    if (!query) return [];
