...
import StoryWizard, {
  STORY_TYPES,
} from "@/features/story/components/StoryWizard";
...
import {
  stripHtml,
  formatMemoryDate,
} from "@/features/story/components/storyListFormatters";
...
function StoryGrid({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {stories.map((story) => {
        const typeInfo =
          STORY_TYPES.find((t) => t.value === story.storyType?.toLowerCase()) ||
          STORY_TYPES[5];
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center gap-1.5">
                {story.privacyLevel === "private" && (
                  <Lock className="w-3 h-3 text-[#5A3E4C]/30" />
                )}
                {story.privacyLevel === "friends_only" && (
                  <Users className="w-3 h-3 text-[#5A3E4C]/30" />
                )}
                {story.privacyLevel === "public" && (
                  <Globe className="w-3 h-3 text-[#5A3E4C]/30" />
                )}
                <span
