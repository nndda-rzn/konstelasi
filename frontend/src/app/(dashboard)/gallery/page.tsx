...
function GalleryContent() {
  ...
  const filteredImages = useMemo(() => {
    ...
    if (filter === "canvas")
      return allImages.filter((img: any) => img.note?.canvas);
    if (filter === "story")
      return allImages.filter((img: any) => img.note?.story);
    return allImages;
  }, [allImages, filter]);
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-sm font-semibold text-[#3F2A35]/60">
              Belum ada media
            </p>
              ...
              <div className="flex items-center justify-between border-t border-white/10 px-5 py-3.5">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-white/90">
                    {selectedImage.note?.title || "Untitled"}
                  </p>
                  <div className="mt-1 flex items-center gap-2 text-[10px] text-white/50">
                    {selectedImage.note?.story && (
                      <span>{selectedImage.note.story.title}</span>
                    )}
                    {selectedImage.note?.canvas && (
                      <span>{selectedImage.note.canvas.name}</span>
                    )}
                    <span>
